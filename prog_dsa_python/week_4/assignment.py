...
def orangecap(scores):
  d = {}
  for match, record in scores.items():
    for player, score in record.items():
      if player not in d:
        d[player] = 0
      d[player] += score
  max_score = max(d.values())
  return [(key, value) for key, value in d.items() if value==max_score][0]
...
def addpoly(p1,p2):
  ...
  return [(value,key) for (key, value) in sorted_d if value!=0]
...
def multpoly(p1,p2):
  ...
  return [(value,key) for (key,value) in d_sorted if value!=0]